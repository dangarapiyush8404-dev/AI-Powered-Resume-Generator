...
def build_resume_buffer(data):
    ...
    # Top banner (right side only to keep left panel clean)
    p.setFillColorRGB(*accent)
    p.rect(200, 760, 395, 82, fill=True, stroke=False)

    # Portrait circle placeholder on left panel near top
    p.setFillColorRGB(1, 1, 1)
    p.setStrokeColorRGB(0.65, 0.65, 0.65)
    p.setLineWidth(1)
    p.circle(100, 740, 60, fill=True, stroke=True)
    ...
    if photo_url:
        try:
            ...
            if image_bytes:
                ...
                # Place photo into placeholder area
                p.drawImage(image_reader, 40, 680, width=120, height=120, preserveAspectRatio=True, mask="auto")
                photo_drawn = True
        except Exception:
    ...
    if not photo_drawn:
        ...
        if initials:
            ...
            p.drawCentredString(100, 730, initials)

    # Name and title (right side, near top banner)
    p.setFillColorRGB(0, 0, 0)
    p.setFont("Helvetica-Bold", 28)
    p.drawString(220, 780, data.get("name", "Your Name"))
    p.setFont("Helvetica", 13)
    p.drawString(220, 760, data.get("profession", "Aspiring Professional"))
    ...
    def draw_left_section(title, lines, y_start):
        ...
        for line in lines:
            ...
            if text_line:
                p.drawString(24, y, f"• {text_line}")
            y -= 12
        return y - 10

    y_left = 500  # start sufficiently below contact section
    # Data is split into lines; if blank no bullets will appear.
    y_left = draw_left_section("Education", data.get("education", "").split("\n"), y_left)
    y_left = draw_left_section("Skills", data.get("skills", "").split("\n"), y_left)
    y_left = draw_left_section("Language", data.get("language", "").split("\n"), y_left)
